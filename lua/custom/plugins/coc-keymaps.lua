...
return {
  {
    ...
    config = function()
      ...
      -- Direct key for Code Action (Alt+a)
      vim.api.nvim_set_keymap('n', '<A-a>', '<Plug>(coc-codeaction)', { silent = true })
      vim.api.nvim_set_keymap('x', '<A-a>', '<Plug>(coc-codeaction-selected)', { silent = true })

      -- Quick fix shortcut (still useful)
